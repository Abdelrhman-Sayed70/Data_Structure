...
T LinkedList<T>::at(int pos) {
	// we need to iterate over all linkedlist using pointer : this pointer will point to node 
...
template<class T>
void LinkedList<T>::myswap(int n, int m) {
	n--; m--;
	assert(n >= 0 and n < count and m>0 and m < count);
	Node<T>* tmp1 = head;
	Node<T>* tmp2 = head;
	for (int i = 0; i < n; i++) {
		tmp1 = tmp1->nextpointer;
	}
	for (int i = 0; i < m; i++) {
